...
class SvgColor:
    ...
    def __init__(self, text: str):
        ...
        m = re.fullmatch(
            r"rgba *\( *([0-9][0-9]*%?) *, *([0-9][0-9]*%?) *, *([0-9][0-9]*%?) *, *([0-9][0-9]*%?) *\)",
            text,
        )
        if m:
            self.red = self.decimal_value(m.group(1))
...
class Params:
    ...
    def __init__(self, json_file: str, values: SvgValues):
        self.params = {}
        self.default = self.global_default
        if json_file is not None:
            with values.config_open(json_file) as file:
                self.set_params(json.load(file), values)

    def make_param(self, param, color=None):
        # order, color and name are required parameters
        order = param["order"]
        if color is None:
        ...
        feed = self.default.feed
        z_feed = self.default.z_feed
        speed = self.default.speed
        passes = self.default.passes
        step = self.default.step
    ...
    def set_params(self, json, values: SvgValues):
        ...
        for key, value in json.items():
            if key == "params":
                for param in value:
                    p = self.make_param(param)
                    self.params[p.color] = p
            elif key == "default":
                self.default = self.make_param(value, color="default")
...
def stroke_to_gcode(gcode: GCode, path: svgelements.Path, param: Param, matrix: Matrix):
    ...
    if gcode.values.verbose:
        print(
            'path using "%s" feed %f z_feed %r speed %f passes %d'
            % (param.name, param.feed, param.z_feed, param.speed, param.passes)
        )
    gcode.set_feed(param.feed)
    gcode.set_z_feed(param.z_feed)
    ...
    for i in range(param.passes):
        for seg in path:
            if isinstance(seg, svgelements.Move):
                ...
                draw.curve2(seg.control.x, seg.control.y, seg.end.x, seg.end.y)
...
def scan_to_gcode(
    gcode: GCode, paths: list[svgelements.Path], param: Param, matrix: Matrix
):
    ...
    for path in paths:
        for seg in path:
            if isinstance(seg, svgelements.Move):
                ...
                edraw.curve2(seg.control.x, seg.control.y, seg.end.x, seg.end.y)
            elif isinstance(seg, svgelements.CubicBezier):
                edraw.curve(
                    seg.control1.x,
                    seg.control1.y,
    ...
    if gcode.values.verbose:
        print(
            'path using "%s" feed %f z_feed %r speed %f passes %d step %f'
            % (
                param.name,
                param.feed,
                param.z_feed,
...
def Args():
    parser = argparse.ArgumentParser(add_help=False, description="Convert SVG to Gcode")
    Device.args(parser)
    parser.add_argument(
        "-p", "--params", action="store", help="Parameter file name", default=None
    )
    parser.add_argument(
        "-x",
        "--x-off",
    ...
    parser.add_argument("file", nargs="*", help="SVG input files")
...
def main():
    ...
    if device.y_invert and values.bounds:
        matrix = matrix.translate(
            0, values.bounds.bottom_right.y + values.bounds.top_left.y
        )
    ...
    for svg in svgs:
        for e in svg.elements():
            ...
            fill = e.values["fill"]
            if (
                isinstance(e, svgelements.Rect)
                or isinstance(e, svgelements.Circle)
                or isinstance(e, svgelements.Ellipse)
            ):
