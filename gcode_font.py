...
class Glyph:
    ...
    def measure_ink(self, width: float, flatness: float) -> TextMetrics:
        ...
        if (
            measure_calls.min_x > measure_calls.max_x
            or measure_calls.min_y > measure_calls.max_y
        ):
...
class Font:
    ...
    def add_svg_glyph(self, element, missing) -> float:
        ...
        if path_string is not None:
            ...
            for p in path:
                ...
                if isinstance(p, Move):
                    ...
                    outline += (
                        "c",
                        chkfloat(p.control1.real),
                        chkfloat(-p.control1.imag),
                        chkfloat(p.control2.real),
                        chkfloat(-p.control2.imag),
                        chkfloat(p.end.real),
                        chkfloat(-p.end.imag),
                    )
        ...
        self.glyphs[ucs4] = Glyph(
            ucs4, width, outline, flatness=self.units_per_em / 1e5
        )
